...
void PersistentTable::truncateTableUndo(TableCatalogDelegate* tcd,
    ...
    if (originalTable->m_tableStreamer != nullptr) {
...
void PersistentTable::compact(void* dst, void const* src, bool releasable) {
    ...
    if (releasable) {
        allocatorFinalize(dst);            // changes on dst is not captured by hook
        allocatorCopier(dst, src);
    } else if (m_schema->getUninlinedObjectColumnCount() > 0 && allocator().frozen()) {
        // frozen, and src lies in frozen region:
        // dst cannot be finalized.
        allocatorCopier(dst, src);
    } else {
...
void PersistentTable::finalizeRelease() {
    using mt = typename storage::CompactingChunks::movement_type;
    allocator().template remove_force<storage::truth>(
            [this] (vector<mt> const& tuples) {    // deep copy when frozen
        for_each(tuples.cbegin(), tuples.cend(), [this](mt const& m) {
                    compact(m.destination(), m.source(), m.sourceReleasible());
                });
    });
    m_invisibleTuplesPendingDeleteCount = 0;
}
...
void PersistentTable::insertTupleCommon(TableTuple const& source, TableTuple& target,
    ...
    if (!conflict.isNullTuple()) {
        throw ConstraintFailureException(this, source, conflict, CONSTRAINT_TYPE_UNIQUE,
                delayTupleDelete ? &m_surgeon : nullptr);
    }
...
void PersistentTable::deleteTuple(TableTuple& target, bool fallible, bool removeMigratingIndex) {
    UndoQuantum* uq = ExecutorContext::currentUndoQuantum();
    bool createUndoAction = fallible && uq != nullptr;
...
std::string PersistentTable::debug(const std::string& spacer) const {
    ...
    for (int index_ctr = 0; index_ctr < m_indexes.size(); ++index_ctr) {
        if (m_indexes[index_ctr]) {
            ...
            // Primary Key
            //
            if (m_pkeyIndex != nullptr && m_pkeyIndex->getName().compare(m_indexes[index_ctr]->getName()) == 0) {
                buffer << " [PRIMARY KEY]";
            }
            buffer << "\n";
        }
...
void PersistentTable::loadTuplesForLoadTable(SerializeInputBE &serialInput, Pool *stringPool,
    ...
    if (uniqueViolationOutput != nullptr) {
...
void PersistentTable::processLoadedTuple(TableTuple& tuple,
    ...
    try {
        if (!ignoreTupleLimit && visibleTupleCount() >= m_tupleLimit) {
            std::ostringstream str;
            str << "Table " << m_name << " exceeds table maximum row count " << m_tupleLimit;
            throw ConstraintFailureException(this, tuple, str.str(), (! uniqueViolationOutput) ? &m_surgeon : nullptr);
        }
...
bool PersistentTable::activateStream(
    ...
    vassert(m_tableStreamer == nullptr || partitionId == m_tableStreamer->getPartitionID());
    if (m_tableStreamer == nullptr) {
...
bool PersistentTable::activateWithCustomStreamer(TableStreamType streamType,
    ...
    vassert(m_tableStreamer == nullptr);
...
int64_t PersistentTable::streamMore(TupleOutputStreamProcessor& outputStreams,
    ...
    if (m_tableStreamer.get() == nullptr) {
        char errMsg[1024];
...
size_t PersistentTable::hashCode() {
    ...
    storage::for_each<PersistentTable::txn_iterator>(allocator(),
         ...
         void *tupleAddress = const_cast<void*>(reinterpret_cast<void const *>(p));
         inputTuple.move(tupleAddress);
         pkeyIndex->addEntry(&inputTuple, nullptr);
    });
...
void PersistentTable::addIndex(TableIndex* index) {
    ...
    storage::for_each<PersistentTable::txn_iterator>(allocator(),
          ...
          void *tupleAddress = const_cast<void*>(reinterpret_cast<void const *>(p));
          inputTuple.move(tupleAddress);
          index->addEntry(&inputTuple, nullptr);
     });
